...
#' )
#'
#' helper_minimize_payments(df)
#'
# Function to calculate payments
helper_minimize_payments <- function(df) {
  ...
  while (i <= nrow(payers)) {
    for (j in 1:nrow(receivers)) {
      if (payers$to_pay[i] == abs(receivers$to_pay[j])) {
        ...
        payments <- payments |>
          tibble::add_row(payer = payers$element[i], receiver = receivers$element[j], amount = payers$to_pay[i])
  ...
  while (i <= nrow(payers) && j <= nrow(receivers)) {
    ...
    # Record the payment
    payments <- payments |>
      tibble::add_row(payer = payers$element[i], receiver = receivers$element[j], amount = pay_amount)
